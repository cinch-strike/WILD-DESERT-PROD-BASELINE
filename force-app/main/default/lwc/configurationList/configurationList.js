...
export default class ConfigurationList extends LightningElement {

//Identifying Transport Job ID
@wire (CurrentPageReference) pageRef;
connectedCallback() {
    registerListener('gettransjobid' ,this.getTransportJob, this);

    registerListener('getdefaultconfiguration' ,this.getConfigurationName, this);
}
...
@api strtransportid;
getTransportJob(detail){
    this.strtransportid = detail.selecteddriverid; 
}

@track strconfigurationname = '';
@track strconfigurationID = '';
getConfigurationName(detail){
    this.strtransportid = detail.selecteddriverid; 
    this.strconfigurationID = detail.selectedconfigurationID; 
    this.strconfigurationname = detail.selectedconfigurationName; 

    alert(this.strtransportid + ' ' + this.strconfigurationID + ' ' + this.strconfigurationname);

    this.selectedAccount = this.strconfigurationname;

    //Currently this is used by Configuration field
    var eventParam = {'selectedvalueid': this.strconfigurationID};
...
@wire(fetchConfiguration, {transportJobID:'$strtransportid'})
